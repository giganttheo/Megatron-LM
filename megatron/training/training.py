...
def train(
    ...
    while iteration < args.train_iters:
        ...
        args.curr_iteration = iteration
        # Keep args.iteration in sync with the local counter so functions
        # called from forward_step (which don't receive iteration) can read
        # the correct value via get_args().iteration.
        ...
        if args.eval_interval and iteration % args.eval_interval == 0 and args.do_valid \
            ...
            if args.perform_rl_step:
                ...
                # Token Superposition Training: temporarily restore baseline
                # seq_length for eval so the schedule uses correct tensor shapes.
                _tst_saved_seq_len = None
                if _tst_in_superposition:
                    _tst_saved_seq_len = args.seq_length
                    args.seq_length = _tst_base_seq_length
                evaluate_and_print_results(prefix, forward_step_func,
                                       ...
                                       non_loss_data_func=non_loss_data_func)
                if _tst_saved_seq_len is not None:
                    args.seq_length = _tst_saved_seq_len
